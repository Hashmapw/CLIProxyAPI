...
func TestCodexExecutorCacheHelper_OpenAIChatCompletions_ManagedPromptCacheKeyFromAPIKey(t *testing.T) {
	...
	httpReq, attemptBody, _, continuity, err := executor.cacheHelper(ctx, sourceFormat, url, nil, req, cliproxyexecutor.Options{
	...
	httpReq2, _, _, _, err := executor.cacheHelper(ctx, sourceFormat, url, nil, req, cliproxyexecutor.Options{
		SourceFormat: sourceFormat,
	}, req.Payload, rawJSON)
	if err != nil {
...
func TestCodexExplicitPromptCacheKeyDoesNotOverwriteAuthScopedSessionCache(t *testing.T) {
	...
	httpReq1, _, _, continuity1, err := executor.cacheHelper(context.Background(), sourceFormat, url, auth, initialReq, opts, initialReq.Payload, initialReq.Payload)
	if err != nil {
	...
	httpReq2, _, _, _, err := executor.cacheHelper(context.Background(), sourceFormat, url, auth, explicitReq, opts, explicitReq.Payload, explicitReq.Payload)
	if err != nil {
	...
	httpReq3, _, _, continuity3, err := executor.cacheHelper(context.Background(), sourceFormat, url, auth, initialReq, opts, initialReq.Payload, initialReq.Payload)
	if err != nil {
...
func TestCodexExecutorCacheHelper_IdentityConfuseRemapsBodyAndHeaders(t *testing.T) {
	...
	httpReq, body, identityState, _, err := executor.cacheHelper(ctx, sourceFormat, url, auth, req, cliproxyexecutor.Options{
		SourceFormat: sourceFormat,
	}, req.Payload, rawJSON)
	if err != nil {
		t.Fatalf("cacheHelper error: %v", err)
