...
func TestClaudeExecutor_ReusesManagedUserIDByDefault(t *testing.T) {
	var userIDs []string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		body, _ := io.ReadAll(r.Body)
	...
	executor := NewClaudeExecutor(&config.Config{})
	auth := &cliproxyauth.Auth{ID: "claude-user-id-reuse-default-main", Attributes: map[string]string{
		"api_key":  "key-123",
		"base_url": server.URL,
	}}
	...
	if userIDs[0] != userIDs[1] {
		t.Fatalf("expected managed user_id to be reused by default, got %q and %q", userIDs[0], userIDs[1])
	}
...
func TestClaudeExecutor_ExecuteRetriesWithNewManagedUserIDAfterError(t *testing.T) {
	var attempts int
	var userIDs []string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attempts++
