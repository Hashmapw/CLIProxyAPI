...
func TestResolveCodexSessionIDUsesCacheWhenSessionAndPromptMissing(t *testing.T) {
	exec := NewCodexExecutor(&config.Config{})
	...
	sessionID1, cacheKey := exec.resolveCodexSessionID(ctx, sdktranslator.FromString("openai-response"), request)
	if strings.TrimSpace(sessionID1) == "" {
		t.Fatalf("first sessionID should not be empty")
	}
	if strings.TrimSpace(cacheKey) == "" {
		t.Fatalf("cacheKey should not be empty")
	}

	sessionID2, cacheKey2 := exec.resolveCodexSessionID(ctx, sdktranslator.FromString("openai-response"), request)
	if cacheKey2 != cacheKey {
		t.Fatalf("cache key mismatch: %q vs %q", cacheKey2, cacheKey)
	}
	if sessionID2 != sessionID1 {
		t.Fatalf("cached sessionID = %q, want %q", sessionID2, sessionID1)
	}

	deleteCodexCache(cacheKey)
	sessionID3, _ := exec.resolveCodexSessionID(ctx, sdktranslator.FromString("openai-response"), request)
	if sessionID3 == sessionID1 {
		t.Fatalf("sessionID should be regenerated after cache deletion, still %q", sessionID3)
	}
}

func TestCodexHttpRequestBackfillsSessionHeaderFromPromptCacheKey(t *testing.T) {
	rt := &captureRoundTripper{status: http.StatusOK, respBody: `{"ok":true}`}
	ctx := context.WithValue(context.Background(), "cliproxy.roundtripper", http.RoundTripper(rt))
	...
	httpReq := &http.Request{
		...
		Body:          io.NopCloser(strings.NewReader(`{"model":"gpt-5","input":"hi","prompt_cache_key":"prompt-from-body"}`)),
		ContentLength: int64(len(`{"model":"gpt-5","input":"hi","prompt_cache_key":"prompt-from-body"}`)),
	}
	...
	if got := strings.TrimSpace(rt.headers.Get("Session_id")); got != "prompt-from-body" {
		t.Fatalf("Session_id = %q, want %q", got, "prompt-from-body")
	}
	if got := gjson.GetBytes(rt.body, "prompt_cache_key").String(); got != "prompt-from-body" {
		t.Fatalf("prompt_cache_key = %q, want %q", got, "prompt-from-body")
	}
}

func TestCodexHttpRequestReusesCachedSessionWhenSessionAndPromptMissing(t *testing.T) {
	deleteCodexCache("gpt-5-default")
	rt := &captureRoundTripper{status: http.StatusOK, respBody: `{"ok":true}`}
	ctx := context.WithValue(context.Background(), "cliproxy.roundtripper", http.RoundTripper(rt))
	...
	firstReq := &http.Request{
		...
		Body:          io.NopCloser(strings.NewReader(`{"model":"gpt-5","input":"hi-1"}`)),
		ContentLength: int64(len(`{"model":"gpt-5","input":"hi-1"}`)),
	}
	firstResp, errFirst := exec.HttpRequest(ctx, auth, firstReq)
	if errFirst != nil {
		t.Fatalf("first HttpRequest error: %v", errFirst)
	}
	if firstResp != nil && firstResp.Body != nil {
		_, _ = io.ReadAll(firstResp.Body)
		_ = firstResp.Body.Close()
	}
	firstSession := strings.TrimSpace(rt.headers.Get("Session_id"))
	if firstSession == "" {
		t.Fatalf("first Session_id should not be empty")
	}

	secondReq := &http.Request{
		Method:        http.MethodPost,
		URL:           reqURL,
		Header:        http.Header{"Content-Type": []string{"application/json"}},
		Body:          io.NopCloser(strings.NewReader(`{"model":"gpt-5","input":"hi-2"}`)),
		ContentLength: int64(len(`{"model":"gpt-5","input":"hi-2"}`)),
	}
	secondResp, errSecond := exec.HttpRequest(ctx, auth, secondReq)
	if errSecond != nil {
		t.Fatalf("second HttpRequest error: %v", errSecond)
	}
	if secondResp != nil && secondResp.Body != nil {
		_, _ = io.ReadAll(secondResp.Body)
		_ = secondResp.Body.Close()
	}
	secondSession := strings.TrimSpace(rt.headers.Get("Session_id"))
	if secondSession != firstSession {
		t.Fatalf("second Session_id = %q, want %q", secondSession, firstSession)
	}
}
