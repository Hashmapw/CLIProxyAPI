...
func TestClaudeExecutor_OpenAITranslationManagedUserIDScopedByAuth(t *testing.T) {
	...
	authA := &cliproxyauth.Auth{
		ID: "claude-user-id-auth-A",
		Attributes: map[string]string{
			"api_key":  "shared-claude-user-id-key",
			"base_url": server.URL,
		},
	}
	authB := &cliproxyauth.Auth{
		ID: "claude-user-id-auth-B",
		Attributes: map[string]string{
			"api_key":  "shared-claude-user-id-key",
			"base_url": server.URL,
		},
	...
	auths := []*cliproxyauth.Auth{authA, authB}
...
func TestClaudeExecutor_ExplicitUserIDDoesNotPolluteManagedCache(t *testing.T) {
	...
	auth := &cliproxyauth.Auth{
		ID: "claude-user-id-explicit-vs-managed",
		Attributes: map[string]string{
			"api_key":  "claude-user-id-explicit-vs-managed-key",
			"base_url": server.URL,
		},
	}
	...
	_, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		...
		OriginalRequest: explicitPayload,
	...
	for i := 0; i < 2; i++ {
		_, err = executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
			...
			OriginalRequest: missingPayload,
			SourceFormat:    sdktranslator.FromString("claude"),
		})
		if err != nil {
...
func TestClaudeExecutor_Execute_RetriesWithNewManagedUserIDAfterError(t *testing.T) {
	...
	executor := NewClaudeExecutor(&config.Config{})
	auth := &cliproxyauth.Auth{
		ID: "claude-user-id-retry-execute",
		Attributes: map[string]string{
			"api_key":  "claude-user-id-retry-execute-key",
			"base_url": server.URL,
		},
	}
...
func TestClaudeExecutor_ExecuteStream_RetriesWithNewManagedUserIDAfterHTTPError(t *testing.T) {
	var attempts int
	var userIDs []string
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attempts++
		...
		if attempts == 1 {
			...
			_, _ = w.Write([]byte(`{"type":"error","error":{"type":"bad_gateway","message":"retry stream"}}`))
			return
		}
	...
	executor := NewClaudeExecutor(&config.Config{})
	auth := &cliproxyauth.Auth{
		ID: "claude-user-id-retry-stream",
		Attributes: map[string]string{
			"api_key":  "claude-user-id-retry-stream-key",
			"base_url": server.URL,
		},
	}
	payload := []byte(`{"model":"claude-3-5-sonnet","messages":[{"role":"user","content":[{"type":"text","text":"hi"}]}]}`)
	...
	if err != nil {
		t.Fatalf("ExecuteStream error after retry path: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("follow-up ExecuteStream error: %v", err)
	}
	for chunk := range result.Chunks {
		if chunk.Err != nil {
			t.Fatalf("unexpected follow-up stream chunk error: %v", chunk.Err)
		}
	}
	...
	if userIDs[0] == userIDs[1] {
		t.Fatalf("expected stream retry to regenerate managed user_id, got identical values %q", userIDs[0])
	}
	if userIDs[1] != userIDs[2] {
		t.Fatalf("expected regenerated stream user_id to be reused after success, got %q and %q", userIDs[1], userIDs[2])
	}
}
